...
libm2k, the library ADI wrote to drive this board: every attribute libm2k
...
That is a translation table between the controls an instrument presents
and the sysfs names a GNU Radio flowgraph needs -- which is exactly the
gap the workshop exists to close.
...
# This is the part a participant can actually use: oscilloscope and
# trigger mean something to them even if an IIO attribute does not.
INSTRUMENTS = [
...
def save(attrs, sources, path=DATA_FILE):
    payload = {
        "_source": "libm2k, the library ADI wrote to drive this board",
        "_upstream": RAW_BASE,
