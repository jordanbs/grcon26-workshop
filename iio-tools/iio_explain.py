...
def show_attr(data, args):
    ...
    for device, channel, attr, parsed in shown:
        ...
        if use:
            ...
            print(para("So this is an instrument control, under its "
                       "sysfs name.", 4))
...
def show_glossary(data, args):
    ...
    add("Generated by `iio-tools/iio_explain.py --glossary`. Everything marked "
    ...
    add("- `Documentation/ABI/testing/sysfs-bus-iio` in the Linux source — "
        ...
        "`./iio-tools/iio_abi_fetch.py --show in_voltage0_raw` prints it.")
    add("- The driver's `iio_chan_spec` table — says which index is which "
        "input.")
    add("- For the M2K: the libm2k source, the library ADI wrote to "
        "drive this board.")
